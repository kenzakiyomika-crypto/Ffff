...
const WakeLockManager = (() => {
  ...
  async function _onVisibility() {
    try {
      ...
      console.warn(`[phase1-training] ${err.message}`);
    }
  }
  ...
  function release() {
    if (_lock) {
      ...
      // [DEV] console.log('[WakeLock] Released manually');
    }
  }
...
const Phase1Training = {
  ...
  async onSessionStart() {
    try {
      ...
      console.warn(`[onSessionStart] ${err.message}`);
    }
  },
  ...
  _showSmartRestBadge(rest) {
    ...
    `;
    safeSetHTML(el, `⏱ REST: ${rest.seconds}s<br><span style="color:#444;font-size:8px">${rest.reason}</span>`;
