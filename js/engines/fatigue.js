...
'use strict';
// FatigueEngine has no external global deps — self-contained
...
const FatigueEngine = (() => {
  ...
  function calcConsecutiveDays(logs) {
    ...
    // Start from latest log date (not today) — prevents streak reset on rest days
    let count = 0;
  ...
  function calcFatigueIndex(logs, asOfDate = null) {
    ...
    const _wd = weekStart.getDay() || 7; // Sunday 0 → 7, prevents jumping to next Monday
...
/* ── v14 Additions: API shims + enhanced status ── */
FatigueEngine.status = (score) => {
  if (score >= 85) return { level:'overtrained', label:'⚠️ OVERTRAINED', color:'#ff2d2d', volumeModifier:0.5  };
  if (score >= 70) return { level:'high',        label:'🔴 FATIGUED',    color:'#ff6b00', volumeModifier:0.75 };
  if (score >= 50) return { level:'moderate',    label:'🟡 MODERATE',    color:'#ffaa00', volumeModifier:0.90 };
  return                   { level:'fresh',       label:'🟢 FRESH',       color:'#00ff88', volumeModifier:1.0  };
};

FatigueEngine.applyWorkout = (currentScore, log) => {
  const vol  = Math.max(0, FatigueEngine.calcSessionLoad(log)); // use engine calc — handles exercises array format
  const dur  = Math.max(0, log.durationMinutes || 0);
  const gain = Math.min(35, Math.round(Math.sqrt(vol / 600 + dur / 60) * 20));
  return Math.min(100, Math.max(0, Math.round(currentScore + gain)));
};

FatigueEngine.applyRecovery = (currentScore, daysPassed = 1) => {
  return Math.min(100, Math.max(0, Math.round(currentScore * Math.pow(0.5, daysPassed / 1.5))));
};

FatigueEngine.clamp = (v) => Math.min(100, Math.max(0, v));
