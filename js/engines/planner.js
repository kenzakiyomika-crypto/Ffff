...
/* ── v14 Additions: UTC date utilities ── */
PlannerEngine.DateUTC = {
  today:       () => new Date().toISOString().split('T')[0],
  dow:         (ds) => new Date(ds + 'T00:00:00Z').getUTCDay(),
  epoch:       (ds) => Date.parse(ds + 'T00:00:00Z'),
  isNewDay:    (last) => {
    const todayE = Date.parse(new Date().toISOString().split('T')[0] + 'T00:00:00Z');
    const lastE  = Date.parse((last || '1970-01-01') + 'T00:00:00Z');
    return todayE > lastE;
  },
};
PlannerEngine.needsDailyReset = (lastResetDateStr) => PlannerEngine.DateUTC.isNewDay(lastResetDateStr);
PlannerEngine.updateDay = (plan, dateStr, dayData) => {
  const newDays = { ...plan._days };
  if (dayData === null) delete newDays[dateStr];
  else newDays[dateStr] = { ...dayData };
  return { ...plan, _days: newDays, totalSessions: Object.values(newDays).filter(d => !d.isRest).length };
};
