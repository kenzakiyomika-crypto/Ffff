...
'use strict';
if (typeof RANKS === 'undefined') throw new Error('[RankEngine] RANKS not loaded — check script load order');
  ...
  function progressToNextRank(pr, currentIndex) {
    ...
    for (const d of details) {
      ...
      if (d.exercise.includes('วิ่ง') || d.exercise.includes('run')) {
        ...
        if (d.current == null) { totalPct += 0; continue; } // null/undefined only — 0 is valid
...
/* ── v14 Additions ── */
RankEngine.calcCapabilityScores = RankEngine.getCapabilityScores;

RankEngine.XP_DAILY_CAP = 200;
RankEngine.applyCap = (earned, alreadyEarnedToday) => {
  return Math.min(earned, Math.max(0, RankEngine.XP_DAILY_CAP - alreadyEarnedToday));
};
